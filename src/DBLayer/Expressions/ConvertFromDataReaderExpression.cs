...
class ConvertFromDataReaderExpression : Expression
{
	...
	public Expression Reduce(IDataContext? dataContext, bool slowMode)
	{
		...
		if (slowMode && DBLayer.Common.Configuration.OptimizeForSequentialAccess)
			return Convert(Call(Constant(columnReader), Methods.DBLayer.ColumnReader.GetValueSequential, _dataReaderParam, Call(_dataReaderParam, Methods.ADONet.IsDBNull, ExpressionInstances.Int32Array(_idx)), Call(Methods.DBLayer.ColumnReader.RawValuePlaceholder)), _type);
		else
			return Convert(Call(Constant(columnReader), Methods.DBLayer.ColumnReader.GetValue, _dataReaderParam), _type);
	}
	...
	internal class ColumnReader
	{
		...
		public object? GetValueSequential(DbDataReader dataReader, bool isNull, object? rawValue)
		{
			...
			catch (DBLayerConvertException ex)
			{
