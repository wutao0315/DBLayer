    ...
    internal class QueryRunner : QueryRunnerBase
    {
        ...
        public override Expression? MapperExpression
        {
            ...
            set
            {
                ...
                if (value != null && DBLayer.Common.Configuration.Linq.TraceMapperExpression &&
        ...
        {
            if (_executionScope != null)
                await _executionScope.DisposeAsync().ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
            ...
            await base.DisposeAsync().ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (executionQuery.PreparedQuery.Commands.Length == 1)
            {
                if (idParam != null)
                {
                    ...
                    await dataConnection.ExecuteNonQueryDataAsync(cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);

                    return idParam.Value;
                }

                return await dataConnection.ExecuteScalarDataAsync(cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
            }
        ...
        public override async Task<IDataReaderAsync> ExecuteReaderAsync(CancellationToken cancellationToken)
        {
            ...
            await _dataConnection.EnsureConnectionAsync(cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);

            base.SetCommand(false);

            InitFirstCommand(_dataConnection, _executionQuery!);

            _dataReader = await _dataConnection.ExecuteDataReaderAsync(_dataConnection.GetCommandBehavior(CommandBehavior.Default), cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
        ...
        public override async Task<int> ExecuteNonQueryAsync(CancellationToken cancellationToken)
        {
            ...
            if (_executionQuery!.PreparedQuery.Commands.Length == 1)
            {
                ...
                return await _dataConnection.ExecuteNonQueryDataAsync(cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
            }
            ...
            for (var i = 0; i < _executionQuery.PreparedQuery.Commands.Length; i++)
            {
                ...
                if (i < _executionQuery.PreparedQuery.Commands.Length - 1 && _executionQuery.PreparedQuery.Commands[i].Command.StartsWith("DROP"))
                {
                    try
                    {
                        await _dataConnection.ExecuteNonQueryDataAsync(cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
                    }
                    catch
                    {
                    }
        ...
        public override async Task<object?> ExecuteScalarAsync(CancellationToken cancellationToken)
        {
            ...
            if (_executionQuery!.PreparedQuery.Commands.Length == 1)
            {
                ...
                return await _dataConnection.ExecuteScalarDataAsync(cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
            }
