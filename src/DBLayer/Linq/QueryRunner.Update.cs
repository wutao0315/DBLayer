...
static partial class QueryRunner
{
	public static class Update<T>
	{
			...
			Type                   type)
		{
			...
			if (fieldCount == 0)
			{
				if (DBLayer.Common.Configuration.Linq.IgnoreEmptyUpdate)
					return null;
			...
			TableOptions           tableOptions)
		{
			...
			var ei               = DBLayer.Common.Configuration.Linq.DisableQueryCache || entityDescriptor.SkipModificationFlags.HasFlag(SkipModification.Update) || columnFilter != null
				...
				: Cache<T>.QueryCache.GetOrCreate(
					...
					{
						entry.SlidingExpiration = DBLayer.Common.Configuration.Linq.CacheSlidingExpiration;
