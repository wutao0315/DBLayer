		...
		if (expression == null)
		{
			...
			if (Builder.MappingSchema.IsScalarType(_elementType))
			{
				...
				else
				{
					...
					var body = Expression.New(Methods.DBLayer.Sql.SqlValueConstructor,
			...
			else
			{
				...
				if (sql.Length == 0)
					throw new DBLayerException($"Entity of type '{_elementType.Name}' as no defined columns.");
			}
		...
		if (!Table.FieldsLookup!.TryGetValue(column.MemberInfo, out var newField))
		{
			...
			if (typeof(DataParameter).IsSameOrParentOf(getter.Body.Type))
			{
				...
				generator.AddExpression(
					Expression.New(
						Methods.DBLayer.Sql.SqlParameterConstructor,
						Expression.Property(variable, Methods.DBLayer.DataParameter.DbDataType),
						Expression.Constant(memberName),
						Expression.Property(variable, Methods.DBLayer.DataParameter.Value)
					));
			}
			else
			{
				generator.AddExpression(Expression.New(Methods.DBLayer.Sql.SqlValueConstructor,
