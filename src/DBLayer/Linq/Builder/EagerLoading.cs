...
internal class EagerLoading
{
	...
	private static Expression GenerateKeyExpression(Expression[] members, int startIndex)
	{
		...
		var constructor  = concreteType.GetConstructor(Array<Type>.Empty) ??
						   throw new DBLayerException($"Can not retrieve default constructor for '{type.Name}'");
	...
	static Expression? ConstructMemberPath(IEnumerable<AccessorMember> memberPath, Expression ob, bool throwOnError)
	{
		...
		foreach (var memberInfo in memberPath)
		{
			...
			{
				if (throwOnError)
					throw new DBLayerException($"Type {result.Type.Name} does not have member {memberInfo.MemberInfo.Name}.");
				return null;
			}
	...
	static Expression? ConstructMemberPath(IEnumerable<MemberInfo> memberPath, Expression ob, bool throwOnError)
	{
		...
		foreach (var memberInfo in memberPath)
		{
			...
			{
				if (throwOnError)
					throw new DBLayerException($"Type {result.Type.Name} does not have member {memberInfo.Name}.");
				return null;
			}
	...
	public static Expression? GenerateDetailsExpression(IBuildContext context, MappingSchema mappingSchema, Expression expression)
	{
		...
		else
		{
			...
			//
			mainQueryWithCollectedKey = TypeHelper.MakeMethodCall(Methods.DBLayer.SelectDistinct, mainQueryWithCollectedKey);
		...
		ParameterExpression masterParam, Expression detailQuery, Expression masterQuery, ExpressionBuilder builder)
	{
		// mark query as distinct
		//
		masterQuery = TypeHelper.MakeMethodCall(Methods.DBLayer.SelectDistinct, masterQuery);
	...
	private static int RegisterPreamblesDetached<TD>(ExpressionBuilder builder, IQueryable<TD> detailQuery)
	{
		...
		var idx = builder.RegisterPreamble(detailQueryPrepared, 
			...
			{
				var query      = (Query<TD>)data!;
				var preambles  = await query.InitPreamblesAsync(dc, expr, ps, ct).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
				var enumerable = query.GetIAsyncEnumerable(dc, expr, ps, preambles);

				return await enumerable.ToListAsync(ct).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
			}
		);
		return idx;
	}
