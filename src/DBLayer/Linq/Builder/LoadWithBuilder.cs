...
class LoadWithBuilder : MethodCallBuilder
{
	...
	protected override IBuildContext BuildMethodCall(ExpressionBuilder builder, MethodCallExpression methodCall, BuildInfo buildInfo)
	{
		...
		if (associations.Length == 0)
			throw new DBLayerException($"Unable to retrieve properties path for LoadWith/ThenLoad. Path: '{path}'");

		if (methodCall.Method.Name == "ThenLoad")
		{
			if (!(table.LoadWith?.Count > 0))
				throw new DBLayerException($"ThenLoad function should be followed after LoadWith. Can not find previous property for '{path}'.");
	...
	static IEnumerable<MemberInfo> GetAssociations(ExpressionBuilder builder, Expression expression, Expression? stopExpression)
	{
		...
		for (;;)
		{
			...
			switch (expression.NodeType)
			{
				...
				case ExpressionType.MemberAccess :
					{
						...
						if (attr == null)
							throw new DBLayerException($"Member '{expression}' is not an association.");
				...
				default :
					{
						throw new DBLayerException($"Expression '{expression}' is not an association.");
					}
			}
		}
	}
