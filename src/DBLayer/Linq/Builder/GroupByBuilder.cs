...
{
	private static readonly MethodInfo[] GroupingSetMethods = new [] { Methods.DBLayer.GroupBy.Rollup, Methods.DBLayer.GroupBy.Cube, Methods.DBLayer.GroupBy.GroupingSets };
					...
					if (selectParamBody is MemberInitExpression mi)
					{
						...
						if (assignment?.Expression.NodeType == ExpressionType.Call)
						{
							...
							if (mc.IsSameGenericMethod(GroupingSetMethods))
							{
								...
								if (mc.IsSameGenericMethod(Methods.DBLayer.GroupBy.Rollup))
									groupingKind = GroupingType.Rollup;
								else if (mc.IsSameGenericMethod(Methods.DBLayer.GroupBy.Cube))
									groupingKind = GroupingType.Cube;
								else if (mc.IsSameGenericMethod(Methods.DBLayer.GroupBy.GroupingSets))
	...
	internal class GroupByContext : SequenceContextBase
	{
		...
		internal class Grouping<TKey,TElement> : IGrouping<TKey,TElement>
		{
				...
				Func<IDataContext,TKey,object?[]?,IQueryable<TElement>> itemReader)
			{
				...
				if (DBLayer.Common.Configuration.Linq.PreloadGroups)
				{
					_items = GetItems();
				}
			}
		...
		class GroupByHelper<TKey,TElement,TSource> : IGroupByHelper
		{
			...
			{
				if (DBLayer.Common.Configuration.Linq.GuardGrouping && !context._isGroupingGuardDisabled)
				{
					if (context.Element.Lambda.Parameters.Count == 1 &&
						context.Element.Body == context.Element.Lambda.Parameters[0])
					{
						var ex = new DBLayerException(
