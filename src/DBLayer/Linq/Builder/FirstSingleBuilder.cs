...
class FirstSingleBuilder : MethodCallBuilder
{
	...
	protected override IBuildContext BuildMethodCall(ExpressionBuilder builder, MethodCallExpression methodCall, BuildInfo buildInfo)
	{
		...
		if (take != 0)
		{
			var takeExpression = DBLayer.Common.Configuration.Linq.ParameterizeTakeSkip
	...
	public class FirstSingleContext : SequenceContextBase
	{
		...
		static void GetFirstElement<T>(Query<T> query)
		{
			...
			query.GetElementAsync = async (db, expr, ps, preambles, token) =>
			{
				...
				await query.GetForEachAsync(db, expr, ps, preambles,
					r => { obj = r; count++; return false; }, token).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
		...
		static void GetFirstOrDefaultElement<T>(Query<T> query)
		{
			...
			query.GetElementAsync = async (db, expr, ps, preambles, token) =>
			{
				...
				await query.GetForEachAsync(db, expr, ps, preambles, r => { obj = r; count++; return false; }, token).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
			...
			query.GetElementAsync = async (db, expr, ps, preambles, token) =>
			{
				...
				await query.GetForEachAsync(db, expr, ps, preambles,
					r =>
					{
						...
						count++;
						return count == 1;
					}, token).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
