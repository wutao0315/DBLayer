...
public class TypeAccessor<T> : TypeAccessor
{
	...
	{
		throw new DBLayerException($"The '{typeof(T).FullName}' type must have default or init constructor.");
	}

	static T ThrowAbstractException()
	{
		throw new DBLayerException($"Cant create an instance of abstract class '{typeof(T).FullName}'.");
	}
