...
{
    private DBLayerConnectionOptions _prebuiltOptions;
    private readonly DBLayerConnectionOptionsBuilder _optionsBuilder = new();
    ...
    public DataContext(string? configurationString)
        : this(new DBLayerConnectionOptionsBuilder()
    ...
    /// <summary>
    /// Creates database context object that uses a <see cref="DBLayerConnectionOptions"/> to configure the connection.
    /// </summary>
    /// <param name="options">Options, setup ahead of time.</param>
    public DataContext(DBLayerConnectionOptions options)
    {
        ...
        if (dataProvider == null)
        {
            ...
            {
                dataProvider = DBLayer.Data.DataConnection.GetDataProvider(options.ProviderName, options.ConnectionString)
                  ?? throw new DBLayerException($"DataProvider '{options.ProviderName}' not found.");
            }
            else if (options.ConfigurationString != null)
                dataProvider = DBLayer.Data.DataConnection.GetDataProvider(options.ConfigurationString);
        }

        if (dataProvider == null)
            throw new DBLayerException($"DataProvider not specified.");
    ...
    protected virtual DataConnection CreateDataConnection(DBLayerConnectionOptions options) => new(options);
    ...
    /// <param name="options">Connection options, will have <see cref="DBLayerConnectionOptions.DbConnection"/> or <see cref="DBLayerConnectionOptions.DbTransaction"/> set.</param>
    /// <returns>New <see cref="DataConnection"/> instance.</returns>
    /// </summary>
    protected virtual DataConnection CloneDataConnection(DataConnection currentConnection, DBLayerConnectionOptions options) => new(options);
