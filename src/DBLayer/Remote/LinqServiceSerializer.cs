			...
			protected int GetType(Type? type)
			{
				...
				if (!ObjectIndices.TryGetValue(type, out var idx))
				{
					...
					else
					{
						...
						Append(DBLayer.Common.Configuration.LinqService.SerializeAssemblyQualifiedName ? type.AssemblyQualifiedName : type.FullName);
					}
			...
			protected Type? ResolveType(string? str)
			{
				...
				if (type == null)
				{
					...
					if (type == null)
					{
						...
						if (type == null)
						{
							if (DBLayer.Common.Configuration.LinqService.ThrowUnresolvedTypeException)
		...
		class QuerySerializer : SerializerBase
		{
			...
			public string Serialize(SqlStatement statement, IReadOnlyParameterValues? parameterValues, IReadOnlyCollection<string>? queryHints)
			{
				...
				if (DelayedObjects.Count > 0)
					throw new DBLayerException($"QuerySerializer error. Unknown object '{DelayedObjects.First().Key.GetType()}'.");
