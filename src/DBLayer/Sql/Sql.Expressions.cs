...
public partial class Sql
{
	...
	class FieldNameBuilder : IExtensionCallBuilder
	{
		public void Build(ISqExtensionBuilder builder)
		{
			...
			if (field == null)
				throw new DBLayerException($"Can not convert expression {builder.Arguments[0]} to field.");
		...
		where T : notnull
	{
		...
		{
			if (dataContext == null)
				throw new DBLayerException("Can not provide information for qualified field name");
	...
	internal static ISqlExpression FieldsExpr<T>( ITable<T> table, Expression<Func<T, object?>> fieldsExpr)
		where T : notnull
	{
	...
	[Extension("", BuilderType = typeof(FieldsExprBuilderDirect), ServerSideOnly = false)]
	internal static ISqlExpression FieldsExpr<T>( ITable<T> table, Expression<Func<T, object?>> fieldsExpr, bool qualified)
		where T : notnull
	{
		var mappingSchema = MappingSchema.Default;
	...
	private static ColumnDescriptor[] GetColumnsFromExpression(Type entityType, LambdaExpression fieldExpr, MappingSchema mappingSchema)
	{
		...
		if (init.Arguments == null || init.Arguments.Count == 0)
			throw new DBLayerException($"Can not extract columns info from expression {fieldExpr.Body}");
		...
		for (var i = 0; i < init.Arguments.Count; i++)
		{
			...
			if (memberInfo == null)
				throw new DBLayerException($"Can not extract member info from expression {init.Arguments[i]}");

			var column = ed.FindColumnDescriptor(memberInfo);

			columns[i] = column ?? throw new DBLayerException($"Can not find column for member {entityType.Name}.{memberInfo.Name}");
		}
	...
	private static ColumnDescriptor GetColumnFromExpression(Type entityType, LambdaExpression fieldExpr, MappingSchema mappingSchema)
	{
		...
		if (memberInfo == null)
			throw new DBLayerException($"Can not extract member info from expression {fieldExpr.Body}");

		var ed     = mappingSchema.GetEntityDescriptor(entityType);
		var column = ed.FindColumnDescriptor(memberInfo);

		if (column == null)
			throw new DBLayerException($"Can not find column for member {entityType.Name}.{memberInfo.Name}");
		return column;
	}
	...
	private class TableNameBuilder : IExtensionCallBuilder
	{
		public void Build(ISqExtensionBuilder builder)
		{
			...
			if (sqlTable == null)
				throw new DBLayerException("Can not find Table associated with expression");
	...
	private class TableOrColumnAsFieldBuilder : IExtensionCallBuilder
	{
		public void Build(ISqExtensionBuilder builder)
		{
			...
			if (sqlField == null)
				throw new DBLayerException("Can not find Table or Column associated with expression");
	...
	{
		throw new DBLayerException("'Sql.TableField' is server side only method and used only for generating custom SQL parts");
	}
	...
	{
		throw new DBLayerException("'Sql.TableOrColumnAsField' is server side only method and used only for generating custom SQL parts");
	}

	[Extension("", BuilderType = typeof(TableAsFieldBuilder))]
	internal static TColumn TableAsField<TEntity, TColumn>( TEntity entity)
	{
		throw new DBLayerException("'Sql.TableAsField' is server side only method and used only for generating custom SQL parts");
	}
		...
		where T : notnull
	{
		...
		if (qualification != TableQualification.None)
		{
			var dataContext = GetDataContext(table);
			if (dataContext == null)
				throw new DBLayerException("Can not provide information for qualified table name");
	...
	{
		throw new DBLayerException("'Sql.TableName' is server side only method and used only for generating custom SQL parts");
	}
	...
	[Extension("", BuilderType = typeof(TableNameBuilderDirect))]
	public static ISqlExpression TableExpr<T>(ITable<T> table, [SqlQueryDependent] TableQualification qualification)
		where T : notnull
	{
	...
	{
		throw new DBLayerException("'Sql.TableExpr' is server side only method and used only for generating custom SQL parts");
	}
	...
	{
		throw new DBLayerException("'Sql.Expr' is server side only method and used only for generating custom SQL parts");
	}
	[Extension("", BuilderType = typeof(ExprBuilder), ServerSideOnly = true)]
	public static T Expr<T>(
