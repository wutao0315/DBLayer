...
public abstract partial class BasicSqlBuilder : ISqlBuilder
{
    ...
    {
        BuildSql(commandNumber, statement, sb, optimizationContext, startIndent, !DBLayer.Common.Configuration.Sql.GenerateFinalAliases && CanSkipRootAliases(statement));
    }
        ...
        foreach (var expr in updateClause.Items)
        {
            ...
            if (expr.Column is SqlRow row)
            {
                if (!SqlProviderFlags.RowConstructorSupport.HasFlag(RowFeature.Update))
                    throw new DBLayerException("This provider does not support SqlRow in UPDATE.");
                if (!SqlProviderFlags.RowConstructorSupport.HasFlag(RowFeature.UpdateLiteral) && expr.Expression is not SelectQuery)
                    throw new DBLayerException("This provider does not support SqlRow literal on the right-hand side of an UPDATE SET.");
            }
    ...
    protected virtual bool? BuildPhysicalTable(ISqlTableSource table, string? alias, string? defaultDatabaseName = null)
    {
        ...
        switch (table.ElementType)
        {
            ...
            case QueryElementType.SqlValuesTable:
                {
                    if (alias == null)
                        throw new DBLayerException("Alias required for SqlValuesTable.");
    ...
    protected virtual void BuildSqlValuesTable(SqlValuesTable valuesTable, string alias, out bool aliasBuilt)
    {
        ...
        else if (IsEmptyValuesSourceSupported)
        {
            ...
            throw new DBLayerException($"{Name} doesn't support values with empty source");
                ...
                if (ext.BuilderType != null)
                {
                    var extensionBuilder = _extensionBuilders.GetOrAdd(
                        ...
                        type =>
                        {
                            ...
                            if (inst is not ISqlExtensionBuilder builder)
                                throw new DBLayerException($"Type '{ext.BuilderType.FullName}' must implement the '{typeof(ISqlExtensionBuilder).FullName}' interface.");
                    ...
                    switch (extensionBuilder)
                    {
                        ...
                        case ISqlTableExtensionBuilder tableExtensionBuilder:
                            ...
                            break;
                        default:
                            throw new DBLayerException($"Type '{ext.BuilderType.FullName}' must implement either '{typeof(ISqlQueryExtensionBuilder).FullName}' or '{typeof(ISqlTableExtensionBuilder).FullName}' interface.");
                    }
                }
    ...
    protected void BuildDataType(SqlDataType type, bool forCreateTable, bool canBeNull)
    {
        ...
        else
        {
            ...
            if (type.Type.DataType == DataType.Undefined)
                ...
                throw new DBLayerException("Database type cannot be determined automatically and must be specified explicitly");
        ...
        if (parameters != null)
        {
            foreach (var p in parameters)
            {
                ...
                if (p.Value is byte[] bytes &&
                    DBLayer.Common.Configuration.MaxBinaryParameterLengthLogging >= 0 &&
                    bytes.Length > DBLayer.Common.Configuration.MaxBinaryParameterLengthLogging &&
                    MappingSchema.ValueToSqlConverter.CanConvert(typeof(byte[])))
                {
                    var trimmed =
                        new byte[DBLayer.Common.Configuration.MaxBinaryParameterLengthLogging];
                    Array.Copy(bytes, 0, trimmed, 0,
                        DBLayer.Common.Configuration.MaxBinaryParameterLengthLogging);
                    ...
                    sb.Append(
                        $"-- value above truncated for logging, actual length is {bytes.Length}");
                         ...
                         DBLayer.Common.Configuration.MaxBinaryParameterLengthLogging >= 0 &&
                         binaryData.Length > DBLayer.Common.Configuration.MaxBinaryParameterLengthLogging &&
                         MappingSchema.ValueToSqlConverter.CanConvert(typeof(Binary)))
                {
                    ...
                    sb.Append(
                        $"-- value above truncated for logging, actual length is {binaryData.Length}");
                         ...
                         DBLayer.Common.Configuration.MaxStringParameterLengthLogging >= 0 &&
                         s.Length > DBLayer.Common.Configuration.MaxStringParameterLengthLogging &&
                         MappingSchema.ValueToSqlConverter.CanConvert(typeof(string)))
                {
                    var trimmed =
                        s.Substring(0,
                            DBLayer.Common.Configuration.MaxStringParameterLengthLogging);
                    MappingSchema.TryConvertToSql(sb, null, trimmed);
                    sb.AppendLine();
                    sb.Append(
