...
class DB2LUWSqlBuilder : DB2SqlBuilderBase
{
	...
	public override StringBuilder BuildObjectName(StringBuilder sb, SqlObjectName name, ConvertType objectType, bool escape, TableOptions tableOptions)
	{
		if (objectType == ConvertType.NameToProcedure && name.Database != null)
			throw new DBLayerException("DB2 LUW cannot address functions/procedures with database name specified.");
		...
		if (name.Database != null && schemaName == null)
			throw new DBLayerException("DB2 requires schema name if database name provided.");
