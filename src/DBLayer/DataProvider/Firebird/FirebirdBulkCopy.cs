...
class FirebirdBulkCopy : BasicBulkCopy
{
		...
		ITable<T> table, BulkCopyOptions options, IEnumerable<T> source)
	{
		...
		if (options.KeepIdentity == true)
			throw new DBLayerException($"{nameof(BulkCopyOptions)}.{nameof(BulkCopyOptions.KeepIdentity)} = true is not supported by Firebird provider. If you use generators with triggers, you should disable triggers during BulkCopy execution manually.");
