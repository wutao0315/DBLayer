...
sealed class ClickHouseSqlBuilder : BasicSqlBuilder
{
    ...
    protected override void BuildMergeStatement(SqlMergeStatement merge) => throw new DBLayerException($"{Name} provider doesn't support SQL MERGE statement");
    protected override void BuildParameter(SqlParameter parameter) => throw new DBLayerException($"Parameters not supported for {Name} provider");
    ...
    private static bool IsValidIdentifier(string identifier)
    {
        if (identifier.Length == 0)
            throw new DBLayerException("Empty identifier");
    ...
    private void BuildTypeName(StringBuilder sb, DbDataType type, bool nullable)
    {
        ...
        switch (type.DataType)
        {
            ...
            case DataType.Enum8:
            case DataType.Enum16: throw new DBLayerException($"Enum type name generation in not supported yet. Use {nameof(ColumnAttribute.DbType)} property to specify enum type explicitly");
            default: throw new DBLayerException($"Cannot infer type name from {type}. Specify DataType or DbType explicitly");
        }
        ...
        if (table.TableOptions.IsTemporaryOptionSet())
        {
            switch (table.TableOptions & TableOptions.IsTemporaryOptionSet)
            {
                ...
                case TableOptions.IsLocalTemporaryStructure | TableOptions.IsLocalTemporaryData:
                    ...
                    break;
                case var value:
                    throw new DBLayerException($"Incompatible table options '{value}'");
            }
        }
        else
        {
    ...
    protected override void BuildDeleteFromClause(SqlDeleteStatement deleteStatement)
    {
        ...
        if (deleteStatement.SelectQuery.From.Tables.Count != 1
            ...
            throw new DBLayerException("Correlated DELETE not supported by ClickHouse");
    ...
    protected override void BuildUpdateQuery(SqlStatement statement, SelectQuery selectQuery, SqlUpdateClause updateClause)
    {
        ...
        if (selectQuery.From.Tables.Count != 1
            ...
            throw new DBLayerException("Correlated UPDATE not supported by ClickHouse");
    ...
    protected override void BuildSetOperation(SetOperation operation, StringBuilder sb)
    {
        switch (operation)
        {
            ...
            default: throw new DBLayerException($"SET operation {nameof(operation)} is not supported by ClickHouse");
        }
    }
