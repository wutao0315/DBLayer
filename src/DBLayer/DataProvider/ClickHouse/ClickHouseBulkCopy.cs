...
sealed class ClickHouseBulkCopy : BasicBulkCopy
{
		...
		where T : notnull
	{
		...
		var bc = await _provider.Adapter.OctonicaCreateWriterAsync!(connection, sql, cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
		await using (bc)
		{
			for (var i = 0; i < columnTypes.Length; i++)
			{
			...
			foreach (var batch in EnumerableHelper.Batch(source, options.MaxBatchSize ?? int.MaxValue))
			{
				...
				await TraceActionAsync(
					...
					async () =>
					{
						await bc.WriteTableAsync(data, rows, cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
						return rows;
			...
			await bc.EndWriteAsync(cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
		...
		where T : notnull
	{
		...
		// thanks C#! (sarcasm)
		var bc = await _provider.Adapter.OctonicaCreateWriterAsync!(connection, sql, cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
		await using (bc.ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext))
		{
			for (var i = 0; i < columnTypes.Length; i++)
			{
				// configure String/FixedString non-default mappings
			...
			await foreach (var batch in batches.WithCancellation(cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext))
			{
				int rows = 0;
				...
				await foreach (var record in batch.WithCancellation(cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext))
				{
					for (var i = 0; i < columns.Count; i++)
					{
						dataConnection.DataProvider.SetParameter(dataConnection, valueConverter, string.Empty, columnTypes[i], columns[i].GetProviderValue(record));
		...
		where T : notnull
	{
		...
		try
		{
			...
			await TraceActionAsync(
				...
				async () =>
				{
					await bc.WriteToServerAsync(rd, cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
					return rd.Count;
				}).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
