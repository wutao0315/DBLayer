...
class PostgreSQLBulkCopy : BasicBulkCopy
{
		...
		ColumnDescriptor[]    columns)
	{
		...
		for (var i = 0; i < columns.Length; i++)
		{
			...
			if (npgsqlType == null && dbTypes[i] == null)
				throw new DBLayerException($"Cannot guess PostgreSQL type for column {columns[i].ColumnName}. Specify type explicitly in column mapping.");
		}
		...
		where T : notnull
	{
		...
		if (connection == null)
			return await MultipleRowsCopyAsync(table, options, source, cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
		...
		var writer = _provider.Adapter.BeginBinaryImportAsync != null
			? await _provider.Adapter.BeginBinaryImportAsync(connection, copyCommand, cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext)
		...
		try
		{
			foreach (var item in source)
			{
				await writer.StartRowAsync(cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
				for (var i = 0; i < columns.Length; i++)
				{
					if (npgsqlTypes[i] != null)
