...
public class DB2ProviderAdapter : IDynamicProviderAdapter
{
	...
	DB2ProviderAdapter()
	{
		...
		var assembly        = DBLayer.Common.Tools.TryLoadAssembly(AssemblyName, ProviderFactoryName);

		if (assembly == null && AssemblyNameOld != null)
		{
			assembly = DBLayer.Common.Tools.TryLoadAssembly(AssemblyNameOld, ProviderFactoryName);
			if (assembly != null)
