	...
	public class BasicBulkCopy
	{
			...
			where T : notnull
		{
			...
			if (options.KeepIdentity == true)
				throw new DBLayerException($"{nameof(BulkCopyOptions)}.{nameof(BulkCopyOptions.KeepIdentity)} = true is not supported by {nameof(BulkCopyType)}.{nameof(BulkCopyType.RowByRow)} mode");
