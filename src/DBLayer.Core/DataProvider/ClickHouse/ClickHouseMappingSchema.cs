...
#region Type converters
		...
		static uint IPAddressToUInt(IPAddress address)
		{
			...
			if (address.AddressFamily == AddressFamily.InterNetworkV6)
				throw new DBLayerConvertException("Cannot convert IPv6 address to UInt32");
...
#region Type to SQL converters (for multi-bindings)
		...
		private static void ConvertByteArray(StringBuilder stringBuilder, SqlDataType dt, byte[] value)
		{
			if (dt.Type.DataType == DataType.IPv6)
			{
				if (value.Length != 4 && value.Length != 16)
					throw new DBLayerConvertException($"IPv6 address should have 4 or 16 bytes, but got {value.Length}");
		...
		private static void ConvertIPAddress(StringBuilder sb, SqlDataType dt, IPAddress address)
		{
			switch ((dt.Type.DataType, address.AddressFamily))
			{
				...
				default:
					...
					throw new DBLayerConvertException($"Unsupported AddressFamily/DataType combination: ({address.AddressFamily} + {dt.Type.DataType})");
			}
		}
		...
		private static void ConvertTimeSpan(StringBuilder sb, SqlDataType dt, TimeSpan value)
		{
			switch (dt.Type.DataType)
			{
				...
				default                        :
					throw new DBLayerConvertException($"Unsupported TimeSpan type mapping: {dt.Type.DataType}");
			}
		}
		...
		private static void ConvertDateTime(StringBuilder sb, SqlDataType dt, DateTime value)
		{
			switch (dt.Type.DataType)
			{
				...
				default                 :
					throw new DBLayerConvertException($"Unsupported DateTime type mapping: {dt.Type.DataType}");
			}
		}
		...
		private static void ConvertByte(StringBuilder sb, SqlDataType dt, byte value)
		{
			...
			var format = GetIntervalLiteralFormat(dt.Type.DataType)
				?? throw new DBLayerConvertException($"Unsupported Byte type mapping: {dt.Type.DataType}");
		...
		private static void ConvertSByte(StringBuilder sb, SqlDataType dt, sbyte value)
		{
			...
			var format = GetIntervalLiteralFormat(dt.Type.DataType)
				?? throw new DBLayerConvertException($"Unsupported SByte type mapping: {dt.Type.DataType}");
		...
		private static void ConvertInt16(StringBuilder sb, SqlDataType dt, short value)
		{
			...
			var format = GetIntervalLiteralFormat(dt.Type.DataType)
				?? throw new DBLayerConvertException($"Unsupported Int16 type mapping: {dt.Type.DataType}");
		...
		private static void ConvertUInt16(StringBuilder sb, SqlDataType dt, ushort value)
		{
			...
			var format = GetIntervalLiteralFormat(dt.Type.DataType)
				?? throw new DBLayerConvertException($"Unsupported UInt16 type mapping: {dt.Type.DataType}");
		...
		private static void ConvertInt32(StringBuilder sb, SqlDataType dt, int value)
		{
			...
			var format = GetIntervalLiteralFormat(dt.Type.DataType)
				?? throw new DBLayerConvertException($"Unsupported Int32 type mapping: {dt.Type.DataType}");
		...
		private static void ConvertUInt32(StringBuilder sb, SqlDataType dt, uint value)
		{
			...
			var format = GetIntervalLiteralFormat(dt.Type.DataType)
				?? throw new DBLayerConvertException($"Unsupported UInt32 type mapping: {dt.Type.DataType}");
		...
		private static void ConvertInt64(StringBuilder sb, SqlDataType dt, long value)
		{
			switch (dt.Type.DataType)
			{
				...
				case DataType.UInt128  :
					if (value < 0)
						throw new DBLayerConvertException($"Value {value} cannot be converted to unsigned UInt128 literal");
					BuildUInt128Literal(sb, value);                                      return;
				case DataType.Int256   : BuildInt256Literal(sb, value);                  return;
				case DataType.UInt256  :
					if (value < 0)
						throw new DBLayerConvertException($"Value {value} cannot be converted to unsigned UInt256 literal");
					BuildUInt256Literal(sb, value);                                      return;
			}

			var format = GetIntervalLiteralFormat(dt.Type.DataType)
				?? throw new DBLayerConvertException($"Unsupported Int64 type mapping: {dt.Type.DataType}");
		...
		private static void ConvertUInt64(StringBuilder sb, SqlDataType dt, ulong value)
		{
			...
			throw new DBLayerConvertException($"Unsupported UInt64 type mapping: {dt.Type.DataType}");
		}
		...
		private static void ConvertBigInteger(StringBuilder sb, SqlDataType dt, BigInteger value)
		{
			...
			throw new DBLayerConvertException($"Unsupported BigInteger type mapping: {dt.Type.DataType}");
		}
		...
		private static void ConvertDateTimeOffset(StringBuilder sb, SqlDataType dt, DateTimeOffset value)
		{
			switch (dt.Type.DataType)
			{
				...
				default                     : throw new DBLayerConvertException($"Unsupported DateTimeOffset type mapping: {dt.Type.DataType}");
			}
		}
		...
		private static void ConvertFloat(StringBuilder sb, SqlDataType dt, float value)
		{
			...
			throw new DBLayerConvertException($"Unsupported Float type mapping: {dt.Type.DataType}");
		}
		...
		private static void ConvertDouble(StringBuilder sb, SqlDataType dt, double value)
		{
			...
			throw new DBLayerConvertException($"Unsupported Double type mapping: {dt.Type.DataType}");
		}
		...
		private static void ConvertDecimal(StringBuilder sb, SqlDataType dt, decimal value)
		{
			...
			throw new DBLayerConvertException($"Unsupported Decimal type mapping: {dt.Type.DataType}");
		}
		...
		private static void ConvertBool(StringBuilder sb, SqlDataType dt, bool value)
		{
			...
			throw new DBLayerConvertException($"Unsupported Boolean type mapping: {dt.Type.DataType}");
		}
#endregion
...
#region Literal generators
		...
		{
			if (precision < 0)
				throw new DBLayerConvertException($"Invalid DateTime64 precision: {precision}");
