...
class OracleBulkCopy : BasicBulkCopy
{
			...
			if (connection != null)
			{
				...
				if (supported)
				{
					...
					using (var bc = _provider.Adapter.BulkCopy.Create(
						...
						options.BulkCopyTimeout ?? (DBLayer.Common.Configuration.Data.BulkCopyUseConnectionCommandTimeout ? connection.ConnectionTimeout : null)))
					{
		...
		ITable<T> table, BulkCopyOptions options, IAsyncEnumerable<T> source, CancellationToken cancellationToken)
	{
		...
		await using (enumerator.ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext))
		{
			// call the synchronous provider-specific implementation
	...
	async Task<BulkCopyRowsCopied> OracleMultipleRowsCopy2Async(MultipleRowsHelper helper, IEnumerable source, CancellationToken cancellationToken)
	{
		...
		foreach (var item in source)
		{
			...
			if (helper.CurrentCount >= helper.BatchSize)
			{
				if (!await ExecuteAsync(helper, list, cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext))
					return helper.RowsCopied;
		...
		if (helper.CurrentCount > 0)
		{
			await ExecuteAsync(helper, list, cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext);
		}
	...
	async Task<BulkCopyRowsCopied> OracleMultipleRowsCopy2Async<T>(MultipleRowsHelper helper, IAsyncEnumerable<T> source, CancellationToken cancellationToken)
	{
		...
		await foreach (var item in source.WithCancellation(cancellationToken).ConfigureAwait(DBLayer.Common.Configuration.ContinueOnCapturedContext))
		{
			list.Add(item!);
		...
		return new CommandInfo(connection, sql, parameters)
			.ExecuteCustom(cmd => _provider.Adapter.ExecuteArray!(
				_provider.TryGetProviderCommand(connection, cmd)
					?? throw new DBLayerException($"AlternativeBulkCopy.InsertInto BulkCopy mode cannot be used with {cmd.GetType()} type. Use OracleTools.UseAlternativeBulkCopy to change mode."),
				iters));
	}
