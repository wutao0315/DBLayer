		...
		if (options.GetProcedures)
		{
			...
			if (procs != null)
			{
				...
				if (GetProcedureSchemaExecutesProcedure && isActiveTransaction)
					throw new DBLayerException("Cannot read schema with GetSchemaOptions.GetProcedures = true from transaction. Remove transaction or set GetSchemaOptions.GetProcedures to false");
