...
public class ExpressionTreeOptimizationContext
{
	...
	{
		return Expression.Call(Methods.DBLayer.SqlExt.Alias.MakeGenericMethod(expression.Type), expression,
		...
		var ex    = body.Transform(
				...
				static (context, wpi) =>
				{
					if (wpi.NodeType == ExpressionType.Parameter && context.parms.ContainsKey((ParameterExpression)wpi))
					{
						...
						throw new DBLayerException($"Can't convert {wpi} to expression.");
					}
