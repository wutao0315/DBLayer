					...
					if (nctor != null)
					{
						...
						if ((recordType & RecordType.CallConstructorOnRead) != 0)
						{
							if (nctor.Members != null)
								throw new DBLayerException($"Call to '{nctor.Type}' record constructor cannot have initializers.");
							else if (nctor.Arguments.Count == 0)
								throw new DBLayerException($"Call to '{nctor.Type}' record constructor requires parameters.");
							else
							{
						...
						{
							if (nctor.Members == null)
								throw new DBLayerException($"Call to '{nctor.Type}' constructor lacks initializers.");
							else
							{
					...
					case ConvertFlags.Field :
						...
						if (expression != null && expression.NodeType == ExpressionType.MemberAccess)
						{
							...
							if (expression == levelExpression)
							{
								...
								if (member == null)
									throw new DBLayerException($"Expression '{expression}' is not a field.");
