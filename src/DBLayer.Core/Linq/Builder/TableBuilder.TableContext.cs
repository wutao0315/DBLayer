		...
		public class TableContext : IBuildContext
		{
			...
			protected virtual Expression BuildQuery(Type tableType, TableContext tableContext, ParameterExpression? parentObject)
			{
				...
				if (IsScalarSet())
				{
					...
					if (info.Length != 1)
						throw new DBLayerException($"Invalid scalar type processing for type '{tableType.Name}'.");
				...
				else
				{
					...
					expr = Expression.Convert(
						Expression.Call(null, Methods.DBLayer.Exceptions.DefaultInheritanceMappingException,
