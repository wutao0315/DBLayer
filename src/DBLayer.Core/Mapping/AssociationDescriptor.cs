	...
	public class AssociationDescriptor
	{
		...
		public Type GetParentElementType()
		{
			if (MemberInfo.MemberType == MemberTypes.Method)
			{
				...
				throw new DBLayerException($"Can not retrieve declaring type form member {methodInfo}");
			}
		...
		public LambdaExpression? GetPredicate(Type parentType, Type objectType)
		{
			...
			if (!string.IsNullOrEmpty(ExpressionPredicate))
			{
				...
				if (members.Length == 0)
					throw new DBLayerException($"Static member '{ExpressionPredicate}' for type '{type.Name}' not found");

				if (members.Length > 1)
					throw new DBLayerException($"Ambiguous members '{ExpressionPredicate}' for type '{type.Name}' has been found");
				...
				if (propInfo != null)
				{
					...
					if (predicate == null)
						throw new DBLayerException($"Property '{ExpressionPredicate}' for type '{type.Name}' should return expression");
				}
				else
				{
					...
					if (method != null)
					{
						if (method.GetParameters().Length > 0)
							throw new DBLayerException($"Method '{ExpressionPredicate}' for type '{type.Name}' should have no parameters");
						...
						predicate = value as Expression;
						if (predicate == null)
							throw new DBLayerException($"Method '{ExpressionPredicate}' for type '{type.Name}' should return expression");
					}
				}
				if (predicate == null)
			...
			{
				throw new DBLayerException($"First parameter of expression predicate should be '{parentType.Name}'");
			}

			if (lambda.Parameters[1].Type != objectType)
				throw new DBLayerException($"Second parameter of expression predicate should be '{objectType.Name}'");

			if (lambda.ReturnType != typeof(bool))
				throw new DBLayerException("Result type of expression predicate should be 'bool'");
		...
		public LambdaExpression? GetQueryMethod(Type parentType, Type objectType)
		{
			...
			if (!(typeof(IQueryable<>).IsSameOrParentOf(lambda.ReturnType) &&
				...
				throw new DBLayerException("Result type of expression predicate should be 'IQueryable<{objectType.Name}>'");
