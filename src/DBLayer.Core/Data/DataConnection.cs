...
public partial class DataConnection : IDataContext, ICloneable
{
	...
	/// </summary>
	public DataConnection() : this(new DBLayerConnectionOptionsBuilder())
	{}
	...
	public DataConnection(MappingSchema mappingSchema) : this(new DBLayerConnectionOptionsBuilder().UseMappingSchema(mappingSchema))
	{
	}
	...
	public DataConnection(string? configurationString, MappingSchema mappingSchema)
		: this(new DBLayerConnectionOptionsBuilder().UseConfigurationString(configurationString ?? DefaultConfiguration!).UseMappingSchema(mappingSchema))
	{
	}
	...
	public DataConnection(string? configurationString)
		: this(new DBLayerConnectionOptionsBuilder().UseConfigurationString(configurationString ?? DefaultConfiguration!))
	{
	}
	...
	public DataConnection(
		...
		: this(new DBLayerConnectionOptionsBuilder().UseConnectionString(providerName, connectionString).UseMappingSchema(mappingSchema))
	{
	}
	...
	public DataConnection(
		string providerName,
		string connectionString)
		: this(new DBLayerConnectionOptionsBuilder().UseConnectionString(providerName, connectionString))
	{
	}
	...
	public DataConnection(
		...
		string        connectionString,
		MappingSchema mappingSchema)
		: this(new DBLayerConnectionOptionsBuilder().UseConnectionString(dataProvider, connectionString).UseMappingSchema(mappingSchema))
	{
	}
	...
	public DataConnection(
		IDataProvider dataProvider,
		string        connectionString)
		: this(new DBLayerConnectionOptionsBuilder().UseConnectionString(dataProvider, connectionString))
	{
	}
	...
	public DataConnection(
		...
		MappingSchema       mappingSchema)
		: this(new DBLayerConnectionOptionsBuilder().UseConnectionFactory(dataProvider, connectionFactory).UseMappingSchema(mappingSchema))
	{
	}
	...
	public DataConnection(
		IDataProvider       dataProvider,
		Func<DbConnection> connectionFactory)
		: this(new DBLayerConnectionOptionsBuilder().UseConnectionFactory(dataProvider, connectionFactory))
	{
	}
	...
	public DataConnection(
		...
		MappingSchema mappingSchema)
		: this(new DBLayerConnectionOptionsBuilder().UseConnection(dataProvider, connection).UseMappingSchema(mappingSchema))
	{
	}
	...
	public DataConnection(
		...
		: this(new DBLayerConnectionOptionsBuilder().UseConnection(dataProvider, connection, disposeConnection))
	{
	}
	...
	public DataConnection(
		...
		MappingSchema  mappingSchema)
		: this(new DBLayerConnectionOptionsBuilder().UseTransaction(dataProvider, transaction).UseMappingSchema(mappingSchema))
	{
	}
	...
	public DataConnection(
		IDataProvider  dataProvider,
		DbTransaction transaction)
		: this(new DBLayerConnectionOptionsBuilder().UseTransaction(dataProvider, transaction))
	{
	}

	private DataConnection(DBLayerConnectionOptionsBuilder builder) : this(builder.Build())
	{
	}

	/// <summary>
	/// Creates database connection object that uses a <see cref="DBLayerConnectionOptions"/> to configure the connection.
	/// </summary>
	/// <param name="options">Options, setup ahead of time.</param>
	public DataConnection(DBLayerConnectionOptions options)
	{
		if (options == null)
		...
		if (!options.IsValidConfigForConnectionType(this))
			throw new DBLayerException(
				$"Improper options type used to create DataConnection {GetType()}, try creating a public constructor calling base and accepting type {nameof(DBLayerConnectionOptions)}<{GetType().Name}>");
		...
		switch (options.SetupType)
		{
			...
			case ConnectionSetupType.DefaultConfiguration:
			{
				...
				if (ConfigurationString == null)
					throw new DBLayerException("Configuration string is not provided.");
			...
			case ConnectionSetupType.ConnectionString:
			{
				if (options.ProviderName == null && options.DataProvider == null)
					throw new DBLayerException("DataProvider was not specified");
				...
				if (options.ProviderName != null)
				{
					...
					if (dataProvider == null)
						throw new DBLayerException($"DataProvider '{options.ProviderName}' not found.");
				}
				else
		...
		RetryPolicy = DBLayer.Common.Configuration.RetryPolicy.Factory != null
			? DBLayer.Common.Configuration.RetryPolicy.Factory(this)
			: null;
		...
		else if (DBLayer.Common.Configuration.Linq.EnableAutoFluentMapping)
		{
	...
	/// <para> - <see cref="IDBLayerSettings.DefaultConfiguration"/>;</para>
	/// <para> - first non-global connection string name from <see cref="IDBLayerSettings.ConnectionStrings"/>;</para>
	...
	/// Initialized with value from <see cref="DefaultSettings"/>.<see cref="IDBLayerSettings.DefaultDataProvider"/>.
	/// </summary>
	/// <seealso cref="DefaultConfiguration"/>
	...
	/// <summary>
	/// Sets trace handler, used for all new connections unless overriden in <see cref="DBLayerConnectionOptions"/>
	/// defaults to calling <see cref="OnTraceInternal"/>.
	/// </summary>
	[Obsolete("Use OnTraceConnection instance property or DBLayerConnectionOptions.OnTrace setting.")]
	...
	/// Configured on the connection builder using <see cref="DBLayerConnectionOptionsBuilder.WithTracing(Action{TraceInfo})"/>.
	...
	void OnTraceInternal(TraceInfo info)
	{
		switch (info.TraceInfoStep)
		{
			...
			case TraceInfoStep.MapperCreated:
			{
				...
				if (DBLayer.Common.Configuration.Linq.TraceMapperExpression && info.MapperExpression != null)
	...
	/// unless <see cref="DBLayerConnectionOptionsBuilder.WithTraceLevel"/> is called on builder.
	...
	/// <remarks>Use <see cref="TraceSwitchConnection"/> when possible, configured via <see cref="DBLayerConnectionOptionsBuilder.WithTraceLevel"/>.</remarks>
	...
	/// will be ignored if <see cref="DBLayerConnectionOptionsBuilder.WriteTraceWith"/> is called on builder
	...
	private static IDBLayerSettings? _defaultSettings;
	...
	/// <seealso cref="IDBLayerSettings"/>
	/// </summary>
	public static IDBLayerSettings? DefaultSettings
	{
#if NETFRAMEWORK
		get => _defaultSettings ??= DBLayerSection.Instance;
#else
	...
	static DataConnection()
	{
		...
		AddProviderDetector(DBLayer.DataProvider.Access    .AccessTools    .ProviderDetector);
		AddProviderDetector(DBLayer.DataProvider.DB2       .DB2Tools       .ProviderDetector);
		AddProviderDetector(DBLayer.DataProvider.Firebird  .FirebirdTools  .ProviderDetector);
		AddProviderDetector(DBLayer.DataProvider.Informix  .InformixTools  .ProviderDetector);
		AddProviderDetector(DBLayer.DataProvider.MySql     .MySqlTools     .ProviderDetector);
		AddProviderDetector(DBLayer.DataProvider.Oracle    .OracleTools    .ProviderDetector);
		AddProviderDetector(DBLayer.DataProvider.PostgreSQL.PostgreSQLTools.ProviderDetector);
		AddProviderDetector(DBLayer.DataProvider.SapHana   .SapHanaTools   .ProviderDetector);
		AddProviderDetector(DBLayer.DataProvider.SqlCe     .SqlCeTools     .ProviderDetector);
		AddProviderDetector(DBLayer.DataProvider.SQLite    .SQLiteTools    .ProviderDetector);
		AddProviderDetector(DBLayer.DataProvider.SqlServer .SqlServerTools .ProviderDetector);
		AddProviderDetector(DBLayer.DataProvider.Sybase    .SybaseTools    .ProviderDetector);
		AddProviderDetector(DBLayer.DataProvider.ClickHouse.ClickHouseTools.ProviderDetector);
	...
	class ConfigurationInfo
	{
		...
		public  IDataProvider  DataProvider
		{
			get
			{
				...
				if (dataProvider == null)
					throw new DBLayerException($"DataProvider is not provided for configuration: {_configurationString}");
	...
	static ConfigurationInfo GetConfigurationInfo(string? configurationString)
	{
		...
		throw new DBLayerException($"Configuration '{configurationString}' is not defined.");
	}
	...
	{
		if (Disposed && (ThrowOnDisposed ?? DBLayer.Common.Configuration.Data.ThrowOnDisposed))
