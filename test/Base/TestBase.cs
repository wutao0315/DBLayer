...
using DBLayer.DataProvider.Informix;
...
#if NET472
			DBLayer.Remote.LinqService.TypeResolver = str =>
			{
				return str switch
				{
		...
		public T[] AssertQuery<T>(IQueryable<T> query)
		{
			...
			var newExpr = expr.Transform(loaded, static (loaded, e) =>
			{
				if (e.NodeType == ExpressionType.Call)
				{
					...
					if (mc.Method.IsGenericMethod && mc.Method.GetGenericMethodDefinition() == Methods.DBLayer.AsSubQuery)
					...
					if (typeof(ITable<>).IsSameOrParentOf(mc.Type))
					{
						...
						if (entityType != null)
						{
							...
							{
								var newCall = DBLayer.Common.TypeHelper.MakeMethodCall(Methods.Queryable.ToArray, mc);
								using (new DisableLogging())
							...
							var queryCall =
								DBLayer.Common.TypeHelper.MakeMethodCall(Methods.Enumerable.AsQueryable,
									itemsExpression);
							return queryCall;
						}
					}
			...
			var empty = DBLayer.Common.Tools.CreateEmptyQuery<T>();
			T[]? expected;
