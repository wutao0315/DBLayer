...
public class a_CreateData : TestBase
{
	...
	{
		using (var conn = DBLayer.DataProvider.Firebird.FirebirdTools.CreateDataConnection(connection))
		{
			conn.Execute(@"
				UPDATE ""Person""
				SET
	...
	{
		using (var conn = DBLayer.DataProvider.SQLite.SQLiteTools.CreateDataConnection(connection))
		{
			conn.Execute(@"
				UPDATE AllTypes
				SET
	...
	{
		using (var conn = DBLayer.DataProvider.Oracle.OracleTools.CreateDataConnection(connection, DBLayer.DataProvider.Oracle.OracleVersion.v11, DBLayer.DataProvider.Oracle.OracleProvider.Managed))
		{
	...
	{
		using (var conn = DBLayer.DataProvider.Informix.InformixTools.CreateDataConnection(connection, ProviderName.Informix))
		{
			conn.Execute(@"
				UPDATE AllTypes
				SET
	...
	{
		using (var conn = DBLayer.DataProvider.Informix.InformixTools.CreateDataConnection(connection, ProviderName.InformixDB2))
		{
			conn.Execute(@"
				UPDATE AllTypes
				SET
