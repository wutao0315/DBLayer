...
#pragma warning restore CS0618
		...
		public void SequenceInsertWithUserDefinedSequenceNameAttribute([IncludeDataSources(TestProvName.AllPostgreSQL)] string context)
		{
			using (var db = GetDataConnection(context))
			{
				var table = new DBLayer.SqlQuery.SqlTable(db.MappingSchema, typeof(PostgreSQLSpecific.SequenceTest1));
		...
		public void SequenceInsertWithoutSequenceNameAttribute([IncludeDataSources(TestProvName.AllPostgreSQL)] string context)
		{
			using (var db = GetDataConnection(context))
			{
				var table = new DBLayer.SqlQuery.SqlTable(db.MappingSchema, typeof(PostgreSQLSpecific.SequenceTest2));
...
#pragma warning restore CS0618
		...
		public void UIntXXMappingTest([IncludeDataSources(TestProvName.AllPostgreSQL)] string context)
		{
			...
			using (var table = db.CreateLocalTable<UIntTable>())
			{
				...
				// test schema
				var schema = db.DataProvider.GetSchemaProvider().GetSchema(db, new DBLayer.SchemaProvider.GetSchemaOptions()
				{
					GetProcedures = false,
					GetTables     = true,
	...
	public class TestPgFunctions
	{
		...
		public DBLayer.ITable<PostgreSQLTests.AllTypes> GetAllTypes()
		{
		...
		public DBLayer.ITable<TestScalarTableFunctionResult> TestScalarTableFunction(int? param1)
		{
		...
		public DBLayer.ITable<TestRecordTableFunctionResult> TestRecordTableFunction(int? param1, int? param2)
		{
		...
		public DBLayer.ITable<TRecord> DynamicTableFunction<TRecord>(string json)
			where TRecord : class
		{
