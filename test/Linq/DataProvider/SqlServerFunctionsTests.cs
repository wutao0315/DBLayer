...
using DBLayer.Tools;
using DBLayer.Tools.DataProvider.SqlServer.Schemas;
...
namespace Tests.DataProvider
{
	...
	public class SqlServerFunctionsTests : TestBase
	{
		...
		{
			using var db = new SystemDB(context);
			var result = db.GetTable<SqlFn.JsonData>(null, DBLayer.Linq.MethodHelper.GetMethodInfo(SqlFn.OpenJson, string.Empty), "{ \"test\" : 1 }").ToArray();
			Console.WriteLine(result);
		...
		{
			using var db = new SystemDB(context);
			var result = db.GetTable<SqlFn.JsonData>(null, DBLayer.Linq.MethodHelper.GetMethodInfo(SqlFn.OpenJson, string.Empty, string.Empty), "{ \"test\" : [ 10, 20 ] }", "$.test").ToArray();
			Console.WriteLine(result);
		...
		{
			using var db = new SystemDB(context);
			var result = db.GetTable<SqlFn.JsonData>(null, DBLayer.Linq.MethodHelper.GetMethodInfo(SqlFn.OpenJson, string.Empty), "[ 10, 20, 30, 40, 50, 60, 70 ]")
				.Where(jd => jd.Key != "2")
