...
namespace Tests.Data
{
	...
	public class DataConnectionTests : TestBase
	{
		...
		public void TestServiceCollection1([IncludeDataSources(TestProvName.AllSQLite, TestProvName.AllClickHouse)] string context)
		{
			...
			collection.AddDBLayer((serviceProvider, options) => options.UseConfigurationString(context));
		...
		{
			var collection = new ServiceCollection();
			collection.AddDBLayerContext<DataConnection>((serviceProvider, options) => options.UseConfigurationString(context));
			var provider = collection.BuildServiceProvider();
		...
		public void TestServiceCollection3([IncludeDataSources(TestProvName.AllSQLite, TestProvName.AllClickHouse)] string context)
		{
			...
			collection.AddDBLayerContext<DbConnection3>((serviceProvider, options) => options.UseConfigurationString(context));
			var provider = collection.BuildServiceProvider();
		...
		{
			public DbConnection1(DBLayerConnectionOptions options) : base(options)
			{
			}
		}
		...
		{
			public DbConnection2(DBLayerConnectionOptions<DbConnection2> options) : base(options)
			{
			}
		}
		...
		{
			public DbConnection3(DummyService service, DBLayerConnectionOptions<DbConnection3> options) : base(options)
			{
			}
		}
		...
		{
			var collection = new ServiceCollection();
			collection.AddDBLayerContext<DbConnection1>((provider, options) => options.UseConfigurationString(context));
			collection.AddDBLayerContext<DbConnection2>((provider, options) => {});
		...
		{
			Assert.Throws<DBLayerException>(() => new DbConnection1(new DBLayerConnectionOptionsBuilder().Build<DbConnection2>()));
		}
