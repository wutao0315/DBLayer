...
namespace Tests.Linq
{
	...
	public class GroupByTests : TestBase
	{
		...
		{
			// https://github.com/igor-tkachev/DBLayer/issues/42
		...
		void CheckGuardedQuery<TKey, TEntity>(IQueryable<IGrouping<TKey, TEntity>> grouping)
			where TKey: notnull
		{
			Assert.Throws<DBLayerException>(() =>
			{
		...
		public void Issue2306Test1([DataSources] string context)
		{
			...
			using (new GuardGrouping(true))
			{
				Assert.Throws<DBLayerException>(() => db.Person.GroupBy(p => p.ID).ToDictionary(g => g.Key, g => g.Select(p => p.LastName).ToList()));
			}
