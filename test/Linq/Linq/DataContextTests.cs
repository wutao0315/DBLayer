...
namespace Tests.Linq
{
	...
	public class DataContextTests : TestBase
	{
		...
		public void ProviderConnectionStringConstructorTest1([DataSources(false, ProviderName.Access, ProviderName.SapHanaOdbc)] string context)
		{
			...
			{
				Assert.Throws<DBLayerException>(() => new DataContext("BAD", db.ConnectionString!));
			}
		...
		class TestDataContext: DataContext
		{
			...
			protected override DataConnection CreateDataConnection(DBLayerConnectionOptions options)
			{
		...
		class NewDataContext : DataContext
		{
			...
			protected override DataConnection CloneDataConnection(DataConnection currentConnection, DBLayerConnectionOptions options)
			{
				CloneCalled++;
